    ...
    <div class="chat-modal-content" v-if="openModal.value" style="background: none">
        ...
        <div class="chat-modal" style="background: none; width: 400px; padding: 0;">
            ...
            <div class="chat-modal-body">
                <VideoPreview :video-list="videoList" v-model:defaultIdx="openModal.idx" v-model="openModal.value" />
            </div>
        </div>
        <!-- </div> -->
    </div>
                        ...
                        <div class="media-group">
                            <ul>
                                <template v-for="(img, idx) in videoList" :key="idx">
                                    <li class="pointer" v-if="videoList.length === 4 || (videoList.length > 4 && idx < 3)">
                                        <img :src="img?.preview_path" @click="onPreviewImage(idx)" />
                                        <video :src="img?.preview_path"></video>
                                    </li>
                                    <li class="pointer" v-if="videoList.length > 4 && idx === 3" @click="onPreviewImage(3)">
                                        <img :src="img?.preview_path" />
                                        <div class="overlay">
                                            <div class="loader">
                    ...
                    <div class="loader">
                      <span>+1</span>
                            ...
                            <div v-if="isLoading" class="overlay"
                                style="position: absolute; top: 50%; left: 49%; transform: translate(-50%, -50%); z-index: 10;">
                                <div class="loader">
                                    <img src="@/assets/icons/loader.svg" />
                                </div>
                            </div>
                ...
                <div class="image-attachment-group">
                    <div class="media-group">
                        <ul>
                            <template v-for="(img, idx) in videoList" :key="idx">
                                <li class="pointer" v-if="videoList.length === 4 || (videoList.length > 4 && idx < 3)"
                                    :style="{ background: isLocalData ? 'rgba(6, 13, 24, 0.7)' : '' }">
                                    <img :src="img?.preview_path" @click="onPreviewImage(idx)" />
                                    <video :src="img?.preview_path"></video>
