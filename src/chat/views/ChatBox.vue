<template>
  <div class="container">
    <div class="chat-detail-wrap">
      <div
        class="chat-head-wrapper d-flex w-100"
      ...
      >
        ...
        <div class="chat-head ml-auto mr-auto">
          <div class="head-sec">
            ...
            <p class="text-center">
              {{ addOrdinalSuffix(Number(route.params.session)) }} Session in
              {{ route.params.year }}
            </p>
          </div>
        </div>
        <div>
          <button class="btn btn-pr" @click="placeCall">Join</button>
        </div>
      </div>
      <div class="chat-area">
        <div class="chat-box" id="scroll_div">
          <div class="conversation-area">
            <div
              :id="`message-${item.id}`"
              v-for="(item, idx) in chat.chat_history"
              :key="idx"
            >
              <ChatDate
                v-if="idx == 0"
              ...
              <ChatDate
                ...
                "
                :date="dateFormat(chat.chat_history[idx].created_at, 'LL')"
              />
              ...
              <UrlMessage
                ...
                :is-self="checkIsLoggedInUser(item.user_id)"
                :sender-name="
                  !checkIsLoggedInUser(item.user_id) ? item.user_name_en : ''
                "
                :time="dateFormat(item.created_at, 'LT')"
              ...
              />
              <VideoMessage
                v-else-if="item.content_type === 'video' || item.content_type === 'recording'"
                :is-self="checkIsLoggedInUser(item.user_id)"
                :sender-name="
                  !checkIsLoggedInUser(item.user_id) ? item.user_name_en : ''
                "
          ...
          <div class="text-box">
            <div class="text">
              <input
                ...
                style="color: black"
                @keydown="sentOnEnter($event)"
                @input="checkData"
              />
            </div>
            <div class="attach-area">
              <div
                class="attach-file pointer"
                @click="optionBoxVisiblity = true"
              >
              ...
              <div class="attach-file pointer" @click="openGallery()">
                ...
                <input
                  ...
                  id="galleryInput"
                  type="file"
                  accept=".jpeg, .jpg, .png, .mp4, .avi, .wmv, .mov"
                  @change="getFile($event)"
                />
              </div>
              <div class="attach-file">
                <button
                  :class="{ 'disable-event': !enableSendBtn }"
                  @click="handleSendMessage"
                >
                  <img src="@/assets/icons/up-btn.svg" alt="" />
                </button>
              </div>
            </div>
          </div>
...
const getLoggedInUserId = computed(() => userStore.getUserData?.user_id);
...
const getImageUrl = async ({ data, type }) => {
  ...
  try {
    ...
    const res =
      type === "video"
        ? await chatService.uploadVideoAttachment(fd)
        : await chatService.uploadAttachment(fd);
...
const onGetChatHistory = async (onscroll) => {
  ...
  try {
    ...
    if (onscroll) {
      ...
      const currentScrollHeight = scrollElement.scrollHeight; // Get current scroll position

      chat.chat_history.unshift(...list);

      setTimeout(() => {
        scrollElement.scrollTo({
          top: scrollElement.scrollHeight - currentScrollHeight,
          behavior: "auto",
        });
      }, 200);
...
const setupSocket = async () => {
  ...
  SocketioService.socket.on("acknowledgement", (data) => {
    if (data.room_uuid != getRoomId.value || data.broadcast_type === "join")
      return;
    ...
    if (obj?.attachments?.length) {
      const findIdx = chat.chat_history.findIndex((item) => item.isLocal);
