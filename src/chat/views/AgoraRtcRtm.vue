      ...
      >
        ...
        >
          ...
          >
            ...
            >
              <img
                src="@/assets/images/video-avtar-icon.jpg"
                alt="Avatar"
                class="avatar"
              />
            </div>
            ...
            <div class="player-info-badge">
              <span v-if="isMute"
                ><img src="@/assets/icons/mic-white-bg-cross.svg" />
              </span>
              <span v-else><img src="@/assets/icons/mic-white-bg.svg" /></span>
    ...
    <div class="chat-call-area">
      <div class="footer d-flex justify-content-between video-chat-footer">
        ...
        <div class="video-chat-button-area">
          ...
          >
            <span v-if="isMute"
              ><img src="@/assets/icons/mic-off-outline.svg"
            /></span>
            <span v-else><img src="@/assets/icons/mic-outline.svg" /></span>
          ...
          >
            <span v-if="isCameraOff"
              ><img src="@/assets/icons/videocam-off-outline.svg"
            /></span>
            <span v-else
              ><img src="@/assets/icons/videocam-outline.svg"
            /></span>
          ...
          <button type="button" class="btn btn-outline-info" disabled>
            <img src="@/assets/icons/vid-user-blue.svg" />
            <span class="counts">
              {{ userCount }}
            </span>
          </button>
          ...
          <!-- <div class="record-box">
              ...
              >
              <span v-if="isRecordingEnabled" class="pointer"><img src="@/assets/icons/record-red.svg"></span>
              <span v-else class="pointer"><img src="@/assets/icons/record.svg"></span>
              </button>
            </div> -->
          <!-- #### =========== SCREEN SHARE ========== #### -->
          <!-- <div class="screen-record-box">
            <button type="button" class="btn btn-info" @click="onShareScreen">
              <span v-if="!isSharingEnabled"
                ><img src="@/assets/icons/screen-share.svg"
              /></span>
              <span v-else
                ><img src="@/assets/icons/screen-share-cut.svg"
              /></span>
          ...
          <button type="button" class="btn calls btn" @click="handleEndCall">
            <img src="@/assets/icons/call-end.svg" />
          </button>
        </div>
      </div>
    </div>
...
import micWhiteBg from '@/assets/icons/mic-white-bg.svg'
import micWhiteBgCross from '@/assets/icons/mic-white-bg-cross.svg'
import videoAvatarIcon from '@/assets/images/video-avtar-icon.jpg'
...
const getUserId = computed(() => userStore.getUserData?.user_id);
...
const muteIcon = `<img src="/img/mic-white-bg-cross.svg">`;
const unmuteIcon = `<img src="/img/mic-white-bg.svg">`;
...
const handleChannelMessage = async (messageData, MemberId) => {
  ...
  console.debug("_________________________________data received", data);
...
const checkVideoAudioAccessibility = async () => {
  try {
    ...
    if (!audioConstraint && !videoConstraint) {
      console.error("no audio/video input devices found.");
      return;
    }
...
const getAgoraResourceId = async () => {
  ...
  try {
    const res = await axios.post(`${BASE_URL}/agora/video/resource-id`, obj);
...
const agoraStartRecording = async () => {
  ...
  try {
    const res = await axios.post(
      `${BASE_URL}/agora/video/start-recording`,
      obj
    );
...
const inserOtherUserOnRecording = async () => {
  const remoteIds = Object.keys(remoteUsers);
  const list = remoteIds.map(Number);
  const obj = {
    channelName: roomId.value,
    userId: UID,
    resourceId: agoraRecordingObj.resource_id,
  ...
  try {
    const res = await axios.post(
      `${BASE_URL}/agora/video/update-recording`,
      obj
    );
    agoraRecordingObj.sid = res.data.sid;
  } catch (error) {
...
const stopRecording = async () => {
  ...
  try {
    const res = await axios.post(`${BASE_URL}/agora/video/stop-recording`, obj);
    channel.sendMessage({
...
.player-wrapper {
  ...
  border: 1px solid #0861ac;
  box-shadow: 10px 4px 34px 0px rgba(0, 0, 0, 0.39);
