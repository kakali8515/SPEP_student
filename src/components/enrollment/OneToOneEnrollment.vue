<template>
    ...
    <div class="primary-region">
        ...
        <!-- ======= SECONDARY REGION ====== -->
        <div class="secondary-region bdrpad">
            ...
            <div class="type-wrapper" v-if="enrollmentObj.primaryRegion">
                ...
                <div class="select-area">
                    <select @change="onSelectSecondaryRegion($event)">
                        ...
                        <option :value="item.area_details_code" :key="idx"
    ...
    <!-- =========## ESP CLASS LESSON ##======== -->
    <div class="primary-region" v-if="isEsp && enrollmentObj.subject">
...
const { fetchSecondaryRegionList, fetchOneToOneClassTimeList, fetchOneToOneClassScheduleList, fetchEspClassLessonList, submitOneToOneNonEspEnrollment, submitOneToOneEspEnrollment, fetchOneToOneSubjectList, fetchRealCourseSeq, fetchClassScheduleList, fetchSmallGroupSubjectList, submitSmallGroupEnrollment } = useEnrollment();
const { getSmallGroupSubjectList, getOneToOneSubjectList, getPrimaryRegionList, getSecondaryRegionList, getClassScheduleListList, getClassTimeList, getClassLevelList, getClassLessonList } = storeToRefs(useEnrollment());
const shiftDropdown = [
    { text: '오전', value: 'AM' },
    { text: '오후 ', value: 'PM' }
];
    ...
    for (let index = 1; index <= _count; index++) {
        switch (index) {
            ...
            case 6: enrollmentObj.day = null;
                break;
            case 7: enrollmentObj.secondaryRegion = "";
                break;
        }
    }
}
const resetLessonMiddleWare = () => {
    if (isEsp.value != 1) return;
    resetLessonSelection();
}
...
const onSubmit = async () => {
    ...
    try {
        ...
        if (res.data.result) {
            ...
            const type = isEsp.value == 1 ? 'oto-esp' : 'oto-non-esp';
